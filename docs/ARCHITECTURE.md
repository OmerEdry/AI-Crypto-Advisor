...
| §9.2a, §2 | "the live provider is implemented but unauthenticated, so it returns **401**", and `BUILD_PLAN` §8 verifies "the **401** is logged with context" | Measured: **404 with an HTML body**, on every v2 plan segment tried (`developer`, `free`, `basic`, `pro`); v1 returns a Cloudflare 403 | The unauthenticated v2 route does not exist rather than refusing credentials, so the documented status was wrong. **Behaviour is unaffected** — the provider rejects any non-2xx before reading the body, so it never attempts to JSON-parse the HTML, and the fallback triggers identically. Two consequences worth stating: the `{plan}` segment is unverifiable in a stronger sense than expected, since no segment produces a different status, and **the README must claim 404, not 401**. The value of §9.2a's design stands and is now measured rather than assumed: the fallback is the path that runs on every request. **The whole live integration is therefore unverifiable, not merely unverified:** with no token, no plan segment reachable and no authenticated reply ever received, the URL shape, the parameter names and the zod schema modelling `results[]` are all written from CryptoPanic's published documentation and cannot be checked against reality from this repository. The provider is deliberately lenient where the documentation is thin — optional fields, a `url`/`original_url` fallback — precisely because a wrong guess about a required field would turn a working response into a parse failure the day a token arrives. Supplying a token is still only an environment variable, but the first authenticated response should be treated as the first real test of this file. |
| §5 | `staticNews.provider.ts` | `static-news.provider.ts` | `CLAUDE.md` §5's kebab-case rule is the more specific one, same precedent as the middleware row above. |
| §9.2a | "~12 curated articles… real titles, real sources, real dates" | 8 articles, capped by the developer; every URL fetched and its headline, publisher and date confirmed before it was written | Feed size is a dial independent of the mechanism §9.2a exists to prove, so it was turned down under time pressure rather than padded. **Provenance, stated because the standard was "verified or dropped":** six articles showed an explicit dateline; two (`news-6`, `news-7`) showed only "N days ago", so their dates come from the publisher's own URL path. For `news-7` the two disagree by one day: the URL path says 2026-08-10, while "4 days ago" read on 2026-08-15 implies the 11th. The stored date is the URL's, 2026-08-10. That is publisher rounding rather than fabrication, and the article was kept deliberately, because dropping it would cost `polkadot` and `hedera-hashgraph` coverage. Two further candidates were dropped because they returned HTTP 403 and could not be verified at all. **Feed captured 2026-08-15; the README carries that date** so the ageing reads as a known property rather than a defect. Ten allowlisted coins have no article, which is what makes Step 8's empty state reachable rather than hypothetical. |
| §15 | — | `resolveJsonModule: true` added to `server/tsconfig.json` | Required to import the curated feed and the meme list. Verified before relying on it that `tsc` copies an imported `.json` into `outDir`, so `npm run build` && `npm start` finds the file — had it not, the app would have worked under `tsx` locally and failed only on the deployed server, which is the worst place to discover it. |
| §9.1 | Recorded at Step 7: `getOrSet` had **no** caller | `news.service.ts` is its one caller | Closes that open note rather than leaving it to be re-derived. The news fetch fits `getOrSet` precisely because its inner function never throws — it falls back to the static provider instead — whereas prices could not use it, needing get → miss → fetch → serve stale. The method stays. |
| `BUILD_PLAN` §10 | Sized **S**, "15–20 curated entries" | Landed at **M** — five files — with **7** entries | Same floor as the Step 6 row: schema, service, controller, routes and the data file before anything else. The count was capped at 8 by the developer and came in at 7 because candidates were dropped rather than padded (below). |
| §5, `BUILD_PLAN` §10 | `middleware/validate.ts` for all input | `?exclude` is parsed **in the memes controller** | Express 5's `req.query` is a getter with no setter, recorded earlier, so `validateBody`'s shape cannot be reused. A `validateQuery` middleware would have exactly one call site in the whole project, which is the single-implementation abstraction `CLAUDE.md` §1 rules out. The zod-issue-to-`details` mapping was extracted from `validate.ts` as `toErrorDetails` so the §6.4 envelope stays identical between the two paths rather than being written twice. |
| §10, `BUILD_PLAN` §10 | "curated entries on stable hosts" | Hotlinked to `i.imgflip.com`, verified per URL | Committing the images would have meant copying work whose rights we do not hold into a public repository; hotlinking makes no copy, and per-URL verification is the mitigation for rot. All seven return HTTP 200 `image/jpeg`, checked against the shipped `memes.json` rather than against the candidate list. **Re-verify before any demo** — a hotlink is the one asset in this project that can break without a deploy. |
| §10 | "curated entries" — selection criteria unstated | **Every image was viewed before inclusion**, and seven candidates were dropped | Titles are not evidence of content, and an unseen image in a portfolio piece is the same class of risk as an unverified headline. Dropped: a 9/11 reference and a probable ethnic joke (on title alone, not opened); a Pepe the Frog image, on the grounds that a contested symbol is not worth the reputational risk here; a joke whose premise is a gender stereotype rather than crypto; one where the template's original caption bleeds through and renders illegibly; one whose "then vs now" lands on surveillance rather than markets; and one not about crypto at all. **`meme-7` is 1045×2235** — an unusually tall portrait kept because it is the strongest of the set, needing `object-contain` and a max-height in the Step 15 card so it does not dominate the section. |
| §6.1 | `GET /api/insight/today → 200 { status, insight, notice? }`, with `insight` unspecified | `insight: { id, content, forDate }` | §4.3 makes `DailyInsight.id` the `itemRef` for INSIGHT votes, so without the id the Step 11 vote has nothing to attach to and Step 15 cannot render an existing vote — the contract as written could not support its own feedback design. `model`, `promptVersion` and `source` stay in the database: they are provenance for later analysis, and the client already learns everything it needs from `status` and `notice`. |
| §4, §8.3 | `forDate DateTime @db.Date // UTC calendar date` — the requirement stated, the construction never specified | `new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()))` | A `Date` built from local parts serialises to the previous or next calendar day for any machine not on UTC, so "today" would differ between the server and the column for several hours daily, and `@@unique([userId, forDate])` would let one user hold two insights for what they experience as one day. Built from UTC parts, the key is the same instant for every caller on earth. Verified: rows store exactly `2026-08-15T00:00:00.000Z`. |
| §8.3, §9.2 | "on failure use the deterministic template… persisted with `source: "fallback"`" | Kept, and **the stickiness named** | §8.3's read-through returns any existing row, so a fallback written at 00:05 UTC is what that user sees until the next UTC day — a transient rate limit becomes a whole-day template. Accepted rather than fixed: an upgrade-on-next-request path would spend scarce quota re-attempting and adds a branch, and the template is honest content rather than an error. **Two consequences:** the notice deliberately does not invite a retry, because retrying returns the identical row and telling the user otherwise would be false; and **testing with `LLM_ENABLED=false` writes a template row against the real account for that UTC day**, so a demo afterwards needs a different account or a deleted row. |
| §12, §8.3 | Read-through then generate | `create`, catching `P2002` and re-reading the winning row | Two concurrent first-requests of the day both miss the read and both generate; the unique constraint decides which row exists, and without the catch the loser returns a 500 on a request that actually succeeded. Same pattern as the duplicate-email row above — a `P2002` that carries business meaning belongs at the service boundary. **The residual, stated rather than papered over:** this makes the *row* correct, not the *quota* — the loser's LLM call has already been spent. Closing that needs an in-process map of in-flight generations keyed by user and date, which was judged not worth the machinery for a race that needs two simultaneous first-requests. |
| §9.3 | "Every outbound call has an explicit timeout — 10s for the LLM" | Kept, and **measured firing on an ordinary call** | The first real generation of this step timed out at `elapsedMs: 10014` against a `:free` model and fell back to the template, while the next call seconds later succeeded in about a second. Free-tier inference latency is variable and routinely brushes 10s. **Combined with the row above this is the sharpest edge in the feature:** a slow first request does not merely degrade one render, it persists the template for that user's whole UTC day. The timeout is left at §9.3's value — raising it trades a fast honest fallback for a slow uncertain wait, and the fallback is genuinely usable — but this is the number to revisit first if insights are template-heavy in practice. |
| §9.3 | "10s for the LLM" | **20s**, raised after the measurement in the row above | Revisited immediately rather than left for later: at 10s the abort was observed on a normal `:free` generation, and sticky per-day persistence converts that abort into a template for the rest of the user's UTC day — including on a cold account during a live demo, which is precisely when the generated insight is the thing worth showing. The trade is stated plainly: a longer worst-case wait on the one request per user per day that generates, against a lower chance of persisting a template no refresh can replace. Every other request that day reads the stored row and waits for nothing. **Note the constant also appears in `CLAUDE.md` §4**, which still reads 10s; that document is the developer's and is not edited here, so the two now disagree until they reconcile it. |
| §8.2 | Prompt includes "Recent feedback signal" | `PROMPT_VERSION = 'v1'` **without** that line | Feedback does not exist until Step 11, which `BUILD_PLAN` says wires the summary in. Recorded so it is not mistaken for an omission: adding the line changes the prompt, so Step 11 must bump `PROMPT_VERSION` to `v2` — that is exactly what per-row versioning is for, and leaving it at `v1` would make two different prompts indistinguishable in the data. Also decided here: `NFT_COLLECTOR` reads the 24h window with day traders, since a collector tracks moves rather than multi-week trends, and §8 assigned it no window. **PII:** the prompt carries investor type, coin ids, content-type enums and public price percentages — no name, email or user id reaches OpenRouter. |
| §6.1, §4.4 | `GET /api/feedback/summary → 200 { summary[] }`, element shape unspecified | `{ sectionType, vote, total, lastVotedAt }` — `investor_type` selected by the SQL and dropped by the service mapper | §4.4's statement is reproduced verbatim because the specification is not the implementer's to edit, and the mapper drops the column. Written fresh the join would not be there at all: the query pins one user, so `investor_type` is constant across every row, and the client already holds it from `GET /api/preferences`. Surfacing it per element would denormalise the envelope to no purpose. The same is true of `JOIN users u`, which the foreign key on `feedback.user_id` already guarantees. |
| §6.1, §10.5, BUILD_PLAN §15 | `{ summary[] }` alone | `{ summary, votes }`, `votes` from `prisma.feedback.findMany` selecting `sectionType, itemRef, vote` | The documented shape could not serve the requirement it exists to feed: the aggregation groups by `(sectionType, vote)` and discards `itemRef`, so after a refresh the dashboard cannot know the user already voted on a specific item — and §10.5's optimistic voting has nothing to render an existing vote from. One route, one dashboard round trip, no change to §4.4, no endpoint outside §6.1. Found at Step 11 rather than at Step 15, which is the difference between ten minutes here and a backend detour in the primary surface. |
| §9.3, §8.2 | Failures logged with context; nothing logs a success | `logger.info('Built insight prompt', { promptVersion, prompt })` on the generate path | §8.2's PII claim was previously an assertion in this table with nothing to check it against. At most one line per user per UTC day — every later request that day reads the stored row — so the cost is negligible and the claim becomes readable: what appears in that line is exactly what leaves for OpenRouter. A claim you can read in a log is worth more than one asserted in a table. |
| §3.4, §9.1 | Raw SQL "expressed directly"; the `as` in `lib/cache.ts` recorded as the server's only cast, to be revisited if a second asked the same excuse | `prisma.$queryRaw<VoteSummaryRow[]>` — the second, and it is that same class of assertion | The generic parameter is the caller's promise about the row shape; Prisma cannot infer it from a template string. **The alternative not taken:** parsing the returned rows with zod converts the assertion into a runtime check, which is strictly stronger — the database is the one input this project trusts without validating. It was declined under time pressure, not on merit, and that is worth recording more than the cast is worth avoiding. `COUNT(*)::int` in §4.4 is load-bearing for a second reason: without the cast Postgres counts as `bigint`, which the driver returns as a `BigInt` that `JSON.stringify` throws on. |
| §8.2, §8.3, NF3 | "Votes measurably change tomorrow's prompt" | Exactly that — and the word *tomorrow* is literal | The loop closes on the next UTC day, not on the next request. §8.3's read-through returns the persisted row, so a vote cast after today's insight was generated reaches the model only when tomorrow's generates. That is a property of the stickiness the quota requires, not a defect, and it is stated here so a reviewer does not read it as a broken feature. A demo that needs to show the loop in one sitting deletes the row for the current UTC day, which is what the Step 11 verification does. |
| §4.3, §6.1 | `itemRef` "a stable per-item identifier" | Bounded and shape-checked; **no existence check** | A vote is a per-user signal, not a foreign key. Validating the target would mean the feedback module calling the news, prices, insight and memes services to ask whether an id is real — four dependencies to reject input that costs one row if wrong. The bound (128) and charset (`^[A-Za-z0-9:_-]+$`) admit every form §4.3 lists and nothing else, which is what keeps an unbounded string out of a unique index. **Consequence:** a vote can outlive its item — a rotated news article leaves an orphan row, which the aggregation still counts by section. Harmless for a signal; it would matter if `itemRef` ever fed a join. |
| §4, §3.3 | Two indexes on `feedback`, one of them `[sectionType, vote]` | Both kept; **no index added for cross-user aggregation** | The compound unique leads with `userId`, which is what every query in this step filters on, and no query groups by `(sectionType, itemRef)` across users yet — `TRAINING_AND_FEEDBACK.md`'s cohort questions are the first that would. Adding that index is online and non-breaking whenever such a query actually arrives, so it is cheaper later than speculative now. |
| §4.4 | `MAX(f.created_at) AS last_voted_at` | Kept as written, and the name named as imprecise | A re-vote updates `vote` and `updated_at` and leaves `created_at` alone, so `last_voted_at` is really *first*-voted-at for any item whose vote changed — verified: a NEWS row re-voted at 12:33:24 reports `12:33:06`. `MAX(f.updated_at)` is what the alias claims. Not changed here, because §4.4 is the specification and this is the row that says so; the field is currently read by nothing — the prompt uses only direction and count. Fix the SQL or the alias before anything renders it. |
| §10.2 | All colour values in `theme.css`, components referencing semantic tokens | `tailwind.config.ts` also maps `borderColor.DEFAULT` to `rgb(var(--color-border))` | Tailwind's preflight sets `border-color` on `*, ::before, ::after` from that key, and its default is the framework's own gray-200 — so every bordered component carried a pale `#e5e7eb` line on a near-black surface. **The defect was invisible to a grep of source files**, which is how it survived four phases: no component, no CSS file and no token contained the value, and nothing renders a border until Step 12 builds the first card. Deliberately declared **without** `<alpha-value>`: preflight substitutes the theme string into static CSS, where the placeholder would have survived into an invalid declaration. Verified by reading the built stylesheet — `#e5e7eb` occurs zero times in the bundle, and `.border-border` keeps its `--tw-border-opacity` channel, so `border-border/50` still composes. |
| §10.2 | `--radius` defined in `theme.css`, not exposed to Tailwind | `borderRadius: { surface: 'var(--radius)' }` → `rounded-surface` | §10.2 requires every token in `theme.css` to have a legal way to be expressed, and `--radius` had none, so the first component to need a corner had a choice between an arbitrary Tailwind step and an inline literal. A **named** key rather than `DEFAULT`: `DEFAULT` silently redefines every bare `rounded` in the project, including ones not yet written, which is a change nobody would attribute to this line. `rounded-surface` pairs with `bg-surface` at the call site. |
| §15, §6.2c | `client/.env` carries `VITE_API_BASE_URL`, "production: /api, via rewrite" | The variable **defaults to `/api`** and is set only in local development | In production `/api` is not an environment-specific value: the rewrite makes the SPA and the API one origin, so the path is a property of the topology rather than of the deployment. Requiring the variable everywhere would convert a fixed fact into the missing-production-variable failure `CLAUDE.md` §2 calls the most common deployment failure — and the failure would be silent, since Vite inlines `undefined` at build time. Local development is the only asymmetric case, and it fails on the first request rather than in production. **Nothing is set on the Vercel dashboard.** |
| §10, BUILD_PLAN §12 | `OnboardingGate.tsx` built in Step 12 | **Deferred to Step 14** | It redirects to `/onboarding`, which did not exist as anything but a stub, so nothing it does could be verified in the session that built it. **The behaviour Step 14 owes, named so the deferral is a plan and not an omission: a user whose `hasCompletedOnboarding` is true, visiting `/onboarding`, is redirected to `/dashboard`.** Until that lands, such a user reaches the onboarding page and can complete it a second time — harmless today, because `PUT /api/preferences` is an idempotent upsert that leaves `onboardingCompletedAt` untouched on a re-save (see the `$transaction` row above), but it is not the specified behaviour. Shape of the fix, confirmed against the built code: one new component plus one wrapping line in `routes.tsx`, since `useAuth` already exposes `hasCompletedOnboarding`. |
| §10.4, BUILD_PLAN §13 | Client-side validation "mirroring the server's rules" | Required fields, email shape and the 8-character register floor — **not** a port of the zod schemas | A second copy of the schema is a second thing to keep in step, and only one of the two actually guards the database; when they drift, the client is the one that will be wrong. What is left out — the 72-character ceiling, zod's exact email grammar — arrives from the server as an inline field error against the right input, because §6.4's `details` array names its field. The login form checks only that a password was typed, matching `loginSchema`'s deliberate `min(1)`: rejecting a short password client-side would answer a failed sign-in differently depending on why it failed, which is precisely what §7.6 exists to prevent. |
| §10.4, §6.4 | `types/api.ts` hand-mirrors the server DTOs; drift produces no compile error | Mirrored, **and parsed at runtime**: `apiRequest` returns `unknown`, and `auth-api.ts` narrows | §10.4 accepts the duplication and names drift as its cost. Returning a caller-chosen generic from the fetch wrapper would have been an unchecked assertion about a payload that crossed a network — the same class of cast recorded twice above for the server — and it is banned outright on this side. Narrowing at the call site costs about thirty lines and converts drift from a silent `undefined` rendered into the page into a failure that names the endpoint. **The honest limit:** the parsers check the fields Steps 12 and 13 read, not the whole DTO, so they catch a rename, not a semantic change. |
| §10, §10.3 | The tree names `ProtectedRoute.tsx` and `OnboardingGate.tsx`; §10.3 also requires `/login` to redirect an authenticated visitor away | Added `PublicOnlyRoute.tsx`; `useAuth` split into `use-auth.ts` | The redirect-away rule had behaviour in §10.3 and no file in §10, and it is the same three-state decision inverted, so it is a component rather than a branch repeated inside each page. The `use-auth.ts` split exists because a file exporting both a provider component and a hook breaks Vite's fast refresh — `react-refresh/only-export-components` flags it, and the warning names a real developer-experience regression rather than a style preference. That file also holds `landingPath`, the single expression deciding where a signed-in visitor belongs, so the post-sign-in redirect and the `/login` guard cannot disagree. |
| §8.1 | "Preferred sections render first and prominently" — the mapping never stated | `MARKET_NEWS`→news, `CHARTS`→prices, `FUN`→meme, **`SOCIAL`→insight**, and the insight clamped to the top two positions whatever the ranking says | Four content types and four sections that **do not map cleanly**, so this is a decision rather than a lookup. Three are obvious. `SOCIAL` is not: there is no social feed in this product, and the insight is the closest thing to what someone choosing it is asking for — commentary rather than raw numbers. The clamp is a second decision and a sharper one: **it overrides the user's stated order.** The insight is the only thing on this page the project actually produced, and a dashboard that renders it under a meme is worse than the one the preference literally asked for. It still loses its emphasis when unranked — it goes quiet, never below the fold. Both are stated here because a table pretending the mapping was obvious would be hiding the two places judgement was applied. |
| §10.2 | `--color-accent: 250 250 250 /* PROVISIONAL */`, `--color-border` and `--color-surface-alt` both `39 39 42` | Warm ink and one amber; Line separated from Raised | The provisional token asked to be replaced by this step. §10.2 warns that near-black plus one acid accent is the generic answer, so the ground moved warm and the single real colour is an amber that is deliberately **never rendered beside a number** — an amber badge next to a red percentage reads as a third status the reader has to decode. Positive and negative were pulled back from full saturation for the same reason: a page of percentages at full chroma reads as a traffic light. **The Line/Raised collision is the same class of defect as the preflight grey above:** two tokens holding one value meant every border on a raised surface was invisible, and no grep of the source could find it — nothing was wrong in any file, and one card was never going to reveal it. It surfaced the moment a coin tile sat on `surface-alt` inside a bordered card. |
| BUILD_PLAN §15 | Header carries "user name, logout, link to edit preferences" | Name and sign-out only | The link has nowhere to go. `/onboarding` is the only preferences UI and the gate correctly redirects an onboarded user away from it, because re-running a wizard is not editing; a settings page is Step 20, which is optional and unbuilt. **What this costs the user, named rather than left implicit: once onboarding is complete there is no way to change assets, investor type or section order from the interface.** The data is editable — `PUT /api/preferences` is idempotent and takes the same body — so what is missing is a screen, not a capability. Building it inside this step was the wrong trade against voting and loading states. |
| §9.4, §8.1 | Section order comes from preferences | First paint renders the default order and reorders when preferences land | Order comes from a request of its own, so the first paint cannot know it. The alternative is blocking the whole page until preferences resolve, which against a sleeping instance is thirty to sixty seconds of empty screen — exactly what §9.4 exists to prevent. The cost is a reflow inside a few hundred milliseconds on a cold load, and the four section queries fire in parallel throughout, so nothing waits on anything. Recorded rather than left for a reviewer to find and assume unnoticed. |
| §10.2 | "Two type roles: one for interface text, one monospaced or tabular-figure for numbers" | One sans face throughout, tabular figures on every number, and the insight's emphasis carried by size, measure and ground | The design brief proposed a system serif for the insight and it was **rejected after looking at it in a browser**: with nothing else on the page set in serif, a reader parses it as an inconsistency before they parse it as hierarchy. A webfont was declined separately — an external request on the page whose entire cold-start story is first paint. §10.2's second role is satisfied by `tabular-nums` rather than by a second family. |
| §10.2 | "Numeric displays get tabular figures" — precision unspecified | **At or above a dollar, two decimal places; below it, four significant digits** | Fixed decimals cannot serve both ends of this list: two places render Shiba Inu as `$0.00`, and six give a four-figure coin four digits of noise that widen the column past every neighbour. Found by looking at the real numbers — `$62,951.00`, `$1,880.08`, `$75.43`, `$0.178389` — where the last one broke the alignment the tabular figures exist to protect. |
| §9.2a | Eight curated articles, "real titles, real sources, real dates" | **Seven.** `news-8` removed | Dated 2026-05-11 against a feed otherwise running 5–14 August, it read as a stale feed rather than a curated one — the opposite of what §9.2a says this content is for. Dropping it was one line of data and no code. **Two consequences:** `solana` now has no article, so eleven of the twenty allowlisted coins have none, which makes the news empty state more reachable rather than less; and the feed's provenance note still holds for every remaining entry. |
| §4, §9.2a | Dates rendered from the values the API returns | Both calendar dates formatted explicitly in UTC | The curated feed sends `publishedAt` as `2026-08-14` with no time, and `DailyInsight.forDate` is a UTC calendar day by construction. `new Date(value).toLocaleDateString()` on either renders **the previous day** for every reader behind UTC — a dashboard quietly dated yesterday, invisible from this timezone and wrong in California. Reproduced before shipping: with `TZ=America/Los_Angeles` the naive call prints `14 Aug 2026` for today's insight where the fix prints `15 Aug 2026`. The day is now taken verbatim and formatted with `timeZone: 'UTC'`. |
| §4.3, §10.5 | `itemRef` per section, one vote per user per item | Built as specified, and **a downvote on a coin is ambiguous in a way the other three are not** | For an article or a meme, thumbs-down means *less like this* and there is nothing else it could mean. For a coin it is genuinely unclear: less of this coin, less of the prices section, or a reaction to the price itself — and the watchlist already answers the first, because the user chose exactly which coins appear. So the PRICES signal is the weakest of the four and would be the first to mislead a ranking model built on this data. **Noticed, not changed:** §4.3 assigns PRICES an `itemRef` and the schema, the summary endpoint and the prompt all already carry it, so removing the control would contradict the design in three places to fix an interpretation problem that belongs in `TRAINING_AND_FEEDBACK.md`. The honest fix is to weight or exclude PRICES votes when the pipeline is built, not to stop collecting them. |
| §8.3 | One insight generated per user per UTC day | Two generations observed in development for a single day; still exactly one row | The row count is guaranteed by `@@unique([userId, forDate])` plus the `P2002` catch, and it held — what doubled was the **LLM call**, which the race row above already names as the residual cost. Two concurrent first-requests both miss the read and both generate. **In development the extra request is cheap to produce:** Vite's HMR re-evaluates `lib/query-client.ts` and a fresh `QueryClient` starts with an empty cache, so the insight query refetches from scratch. StrictMode's double-invocation is the usual suspect and is *not* sufficient on its own — TanStack dedupes concurrent fetches sharing a key — so it can contribute only if the two mounts straddle a cache reset. **In production the StrictMode path is gone entirely**, since React strips the double invocation from production builds, and a single tab will not refetch because `staleTime` runs to the next UTC day. What remains reachable is the genuine race: two tabs, two devices, or a window-focus refetch landing beside a first load. It costs one extra request against a ~50/day account-wide quota and can never produce a second row. **The diagnostic is already in place:** each generation logs one `Built insight prompt` line, so counting those lines against the row count measures it exactly. |
| §6.2b | "The browser never contacts a third-party API" | Coin logos are `<img src>` straight to CoinGecko's CDN | The `image` field was added at Step 7 precisely so a logo would not require a later provider change, so rendering it is the intended use. Worth naming because it is the one outbound request the browser makes to a third party: it carries no key, no cookie and nothing about the user, and it is an image fetch rather than an API call — the rule in §6.2b is about credentials and cache control, both of which stay server-side. The alternative is proxying every logo through Express, which spends our own bandwidth to hide a public PNG. |
